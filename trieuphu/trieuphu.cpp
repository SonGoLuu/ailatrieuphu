...
{  
   ...
   setcolor(GREEN); setlinestyle(1,0,4); 
...
void Result(int r)
{
 ...
 { setfillstyle(SOLID_FILL,BLUE); setcolor(WHITE); setbkcolor(BLUE); settextstyle(GOTHIC_FONT,0,4); 
	 bar(230,330,830,500); outtextxy(395,365,"Ban thua roi"); outtextxy (310,420,"Nhan nut     de thoat"); 
	 setbkcolor(RED);outtextxy (517,420,"X");
 }
 if (r==1)
{ setfillstyle(SOLID_FILL,BLUE); setcolor(WHITE); setbkcolor(BLUE); settextstyle(GOTHIC_FONT,0,4); 
 bar(160,330,900,500); outtextxy(175,365,"Wow ban da tro thanh trieu phu :))"); 
 outtextxy (310,420,"Nhan nut     de thoat"); setbkcolor(RED);outtextxy (517,420,"X");
...
{  int count,cau,r;
	...
	case 1:  cau=1; setbkcolor(LIGHTGRAY); settextstyle(GOTHIC_FONT,0,2); setcolor(14); outtextxy(23,29,"Money: 0$");
        ...
        if (ismouseclick(WM_LBUTTONDOWN)){
			...
			if(x>=600 && x<=1000 && y>=300 && y<=430)
			{setcolor(5); setbkcolor(LIGHTGRAY); settextstyle(GOTHIC_FONT,0,3);  outtextxy(450,50,"Chinh xac!"); count=1; break;}
			if(x>=50 && x<=450 && y>=300 && y<=430 || x>=50 && x<=450 && y>=470 && y<=600 || x>=600 && x<=1000 && y>=470 && y<=600 )
			{setcolor(5); setbkcolor(LIGHTGRAY); settextstyle(GOTHIC_FONT,0,3); outtextxy(450,50,"Sai roi!"); count=0; r=0; Result(r); break;}
		    }}
	case 2:  cau=2; 
		if (count==1){
